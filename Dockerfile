...
WORKDIR $work
RUN git clone --branch "v2018.6" https://github.com/gromacs/gromacs.git

# patch gromacs with plumed and compile
WORKDIR $work/gromacs
RUN plumed patch -p --runtime -e "gromacs-2018.6" \
 && mkdir -p build
WORKDIR $work/gromacs/build
RUN cmake .. -DCMAKE_INSTALL_PREFIX="/usr/local/gromacs" -DGMX_SIMD="AVX2_256" -DGMX_BUILD_OWN_FFTW="off" -DGMX_MPI="on" -DCMAKE_C_COMPILER="mpicc" -DCMAKE_CXX_COMPILER="mpicxx" -DGMX_GPU="on" -DGMX_USE_NVML="off" \
 && make -j "$(nproc)" \
 && make install \
 && rm -rf ../../gromacs
